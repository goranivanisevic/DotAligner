...
if($secStruct) {
  ...
  if(index($consStruc,"-")==0) {
    $consStruc=~s/-/./;
  }
  #$consStruc=$secStruct;

  my $open = () = $consStruc=~/\(/g;
  my $close = () = $consStruc=~/\)/g;

  if(length($$aln[0]->{seq})!=length($consStruc)) {
    die($$aln[0]->{seq}."\n".$consStruc."\n"."Length of secondary structure (" . length($consStruc) . ") differs from alignment length (" . length($$aln[0]->{seq}) . ")!\n");
  }
  if($open!=$close) {
    die("Secondary structure is invalid because the number of opening (" . $open . ") and closing bases (" . $close . ") differ.\n");
  }
}
else {
  open(ALIRNA,"<$ssFile");
...
if($pp) {
	my $bprel = readPp($pp,$consStruc);
	plotAln($aln,$consSeq,$consStruc,$bprel);
}
else {
...
#PETCOFOLD web server version
#   } elsif ( /^(\S+)\s+([A-Z\-]+)\s*\d*$/ ) {
    } elsif ( /^(\S+)\s+([A-Za-z\-&]+)\s*\d*$/ ) {
	  ($seqname,$aln_line) = ($1,uc($2));
	} else {
	  next;
  }
...
######################################################################
#
# readPp(\@aln ref-to-pp_file, $ss string)
#
# Returns array of sequence length with paired reliability for each nucleotide (0..1).
#
######################################################################
...
######################################################################
#
# plotAln(\@aln ref-to-alignment, $consensus string, $ss string, \@bprel ref-to-pp_file (optional))
#
...
sub plotAln{
  ...
  my $imageHeight = int($startY+$numBlock*((@aln+1+$ruler)*$lineStep+$blockStep+(1+$pp)*$consStep+$ssStep+(1+$pp)*$maxConsBar));
  ...
  foreach my $pair (@pairs) {
	...
	if ($nonpairing <=2) {
	  ...
	  if (($pair->{all}->[$row] eq 'AU') or
	    ...
	    my $y=$startY+($block-1)*($lineStep*(@aln+1+$ruler)+$blockStep+(1+$pp)*$consStep+$ssStep+(1+$pp)*$maxConsBar)+($row+$ruler)*$lineStep;
  ...
  while ($currPos<$length) {
    ...
    if ($ruler) {
      ...
      my $and=index($aln[0]->{seq},"&")+1;
      my $rulPos = ($currPos>=$and) ? $currPos-$and : $currPos;

      #$ps .= "(ruler) $namesX $currY string\n";
      my $maxp = $currPos+$columnWidth;
      my $p = 10 * (int(1.5+$rulPos/10));
      my $r = " " x ($p-$rulPos-length("$p"));
      $r .= $p; $p+=10;
      if($and){
	if($currPos<$and){
          $maxp = $and if $maxp>$and;
	  if($and-$currPos<10){
            $p=10000000;
            $r="";
	  }
	...
	$maxp = $length if $maxp>$length;
      ...
      for (;$p<=$maxp; $p+=10) {
	$r .= sprintf("%10d", $p) if $p!=$and;
      }
      if($and && $currPos<$and){
        $maxp = $currPos+$columnWidth-$and;
        $p = 10;
        $r .= " " x (($and-1)%10);
        $r .= "&";
        for (;$p<=$maxp; $p+=10) {
...
#    $ps .= "0 setgray\n";
    ...
    # base paired reliability curve
    if($pp){
      $currY+=$consStep;
      my $yrel=$currY+$maxConsBar-6;
      $ps .= "(reliab_paired) $namesX $yrel string\n";
      $ps .= "0.6 setgray\n";
      for my $col ($currPos..$currPos+$columnWidth-1) {
        my $score=shift @bprel;
    ...
    # conservation curve
    $currY+=$consStep;
    my $ycons=$currY+$maxConsBar-6;
    $ps .= "0 setgray\n";
    $ps .= "(conservation) $namesX $ycons string\n";
    $ps .= "0.6 setgray\n";
    for my $col ($currPos..$currPos+$columnWidth-1) {
...
mycoloraln.pl - colorize an alignment with consensus structure

=head1 SYNOPSIS

  mycoloraln.pl [-r] [-n] [-c columns] [-s structure.ps] [-ss "sec.struct."] [-pp pp_file] file.aln
...
=item B<-ss> I<secondary structure>

input the consensus structure in dot-bracket format
