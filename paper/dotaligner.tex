...
discuss in more detail how the algorithm works.
...
\begin{equation}\label{eq1}
	...
	A, (i,j^\prime) \in A}}} \tau(i,j,i^\prime,j^\prime) + \gamma
	\times L,
\end{equation}
...
\in P_a$ and $(i^\prime,j^\prime) \in P_b$ ($\tau(i,j,i^\prime,j^\prime) = 1 - | P_a(i,j)-P_b(i^\prime,j^\prime) |$), and $\gamma$ is the gap cost
...
\ENDFOR
\ENDFOR
\STATE $A(S_a,S_b) = BACKTRACKING(D)$
...
The robustness of the alignments can be improved by replacing $P(i,i^\prime)$ with 

\begin{equation}
\Psi_{i,i^\prime} = log \frac{P(i,i^\prime)}{p_0} / log \frac{1}{p_0} 
\end{equation}

where $p_0$ is the expected probability for a pairing to occur at random (use
ribosomal basepair substitution matrix from \texttt{Pfold}
\cite{Knudsen:Hein:Pfold_RNA_secon:NAR2003}). $\Psi$ was firstly introduced in
\cite{Will17432929}. The term $ log \frac{P(i,i^\prime)}{p_0}$ is the log-odds
score for having a specific base pairing against the null model of a random
pairing, and $log \frac{1}{p_0}$ is a normalization factor that transforms the
scores to a maximum of 1. $P==1$ results in $\Psi=1$, $P>p_0$ results in
$\Psi>0$, $P==p_0$ results in $\Psi=0$, and $P<p_0$ results in $\Psi<0$. This
transformation gives larger dissimilarities if low basepair probabilities are
involved, but relatively lower dissimilarities if two high basepair probabilities
are compared.

The proposed algorithm can be optimized by different adaptations which will be
